...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        To find the maze distance between any two positions, use: self.distancer.getDistance(pos1, pos2)

        To find the successor position of a position after an action: successorPosition = Actions.getSuccessor(position, action)
        ...
        for beliefs in livingGhostPositionDistributions:
            ...
            for pos, prob in beliefs.items():
                if prob > max_prob:
                    max_prob = prob
                    max_likely_pos = pos
            distance = self.distancer.getDistance(max_likely_pos, pacmanPosition)
            if distance < min_distance:
                min_distance = distance
                current_target_pos = max_likely_pos
        action = None
        for a in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, a)
            if self.distancer.getDistance(successorPosition, current_target_pos) < min_distance:
                action = a
        return action
        "*** YOUR CODE HERE ***"
