    ...
    def observe(self, observation, gameState):
        ...
        if (noisyDistance == None):
            for p in self.legalPositions:
                new_belief[p] = 0.0
            new_belief[self.getJailPosition()] = 1.0
        else:
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                sensor_model = emissionModel[trueDistance]
                if emissionModel[trueDistance] > 0: new_belief[p] = sensor_model*self.beliefs[p]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                # prob = Pr( ghost is at position newPos at time t + 1 | ghost is at position oldPos at time t )
                # self.beliefs[oldPos] = Pr( ghost is at position oldPos at time t)
                new_belief[newPos] += prob * self.beliefs[oldPos]
        new_belief.normalize()
        self.beliefs = new_belief   
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        per_pos = self.numParticles/len(self.legalPositions)
        for p in self.legalPositions:
    ...
    def observe(self, observation, gameState):
        ...
        if (noisyDistance == None):
            ...
            if (total_weight == 0):
                ...
                W.normalize()
                for i in range(self.numParticles):
                    self.particleList.append(util.sample(W))
    def elapseTime(self, gameState):
        """
        ...
        for p in self.particleList:
            ...
            new_particle_list.append(util.sample(newPosDist))
        self.particleList = copy.deepcopy(new_particle_list)
    ...
    def getBeliefDistribution(self):
        ...
        for p in self.particleList:
            particle_distribution[p] +=1
        beliefs = util.Counter()
        for p in particle_distribution.sortedKeys():
            beliefs[p] = float(particle_distribution[p])/float(self.numParticles)
        return beliefs
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        while(len(self.particleList) < self.numParticles):
            for t in tuples:
                if (len(self.particleList) < self.numParticles):
                    self.particleList.append(t)
        ...
        for particle in self.particleList:
            ...
            for i in range(len(noisyDistances)):
                if noisyDistances[i] is None:
                    ...
                    sensor_model *= emissionModels[i][trueDistance]
            W[particle] += sensor_model
        total_weight = W.totalCount()
        if (total_weight== 0):
            self.initializeParticles()
            for i in range(len(self.particleList)):
                for j in range(len(noisyDistances)):
